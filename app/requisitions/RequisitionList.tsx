...
const STATUS_STYLES: Record<string, { bg: string; color: string; label: string }> = {
  ...
  ordered:  { bg: '#E0E4E9', color: '#4E7FB5', label: 'Ordered' },
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.75rem', gap: '0.75rem', flexWrap: 'wrap' }}>
        <div>
          <h1 style={{ fontSize: '1.25rem', fontWeight: 700, color: '#1B3A5C', marginBottom: '2px' }}>Purchase Requisitions</h1>
          <p style={{ color: '#717680', fontSize: '0.875rem' }}>{requisitions.length === 0 ? 'No requisitions submitted yet' : `${requisitions.length} requisition${requisitions.length === 1 ? '' : 's'}`}</p>
        </div>
      ...
      {requisitions.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '4rem 1rem', background: '#fff', borderRadius: '1rem', border: '1px solid #D4D7DC' }}>
          ...
          <p style={{ color: '#717680', fontSize: '0.9375rem', marginBottom: '0.75rem' }}>No purchase requisitions yet.</p>
          ...
          {requisitions.map(req => {
            ...
            return (
              <button key={req.id} onClick={() => setViewing(req)}
                style={{ background: '#fff', borderRadius: '0.875rem', padding: '1rem 1.25rem', border: '1px solid #D4D7DC', cursor: 'pointer', textAlign: 'left', transition: 'box-shadow 0.15s, border-color 0.15s', display: 'block', width: '100%' }}
                onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.boxShadow = '0 4px 16px rgba(27,58,92,0.1)'; (e.currentTarget as HTMLButtonElement).style.borderColor = '#A8B8CC' }}
                onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.boxShadow = ''; (e.currentTarget as HTMLButtonElement).style.borderColor = '#D4D7DC' }}
              >
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '0.75rem', marginBottom: '0.5rem' }}>
                  ...
                  <span style={{ fontSize: '0.75rem', color: '#B0B4B9', flexShrink: 0 }}>{new Date(req.createdAt).toLocaleDateString()}</span>
                </div>
                <p style={{ fontSize: '0.9375rem', color: '#1B3A5C', fontWeight: 600, marginBottom: '0.25rem' }}>{req.title}</p>
                <p style={{ fontSize: '0.8125rem', color: '#717680' }}>{req.lineItems.length} line item{req.lineItems.length === 1 ? '' : 's'} · by {req.user.name}</p>
                {req.adminNote && <p style={{ fontSize: '0.75rem', color: '#717680', marginTop: '0.375rem' }}>Admin: {req.adminNote}</p>}
              </button>
            )
          })}
        </div>
      ...
      {viewing && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', padding: '0', zIndex: 50 }}>
          <div style={{ background: '#fff', borderRadius: '1rem', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', width: '100%', maxWidth: '42rem', maxHeight: '85vh', display: 'flex', flexDirection: 'column', border: '1px solid #D4D7DC' }}>
            ...
            <div style={{ padding: '1.5rem', overflowY: 'auto', overscrollBehavior: 'contain', flex: 1, display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              <div>
                <p style={{ fontSize: '0.75rem', fontWeight: 600, color: '#717680', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.375rem' }}>Justification</p>
                <p style={{ fontSize: '0.9375rem', color: '#1B3A5C', lineHeight: 1.6 }}>{viewing.justification}</p>
              </div>
              <div>
                <p style={{ fontSize: '0.75rem', fontWeight: 600, color: '#717680', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.75rem' }}>Line Items</p>
                <div style={{ border: '1px solid #D4D7DC', borderRadius: '0.625rem', overflow: 'hidden' }}>
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.875rem' }}>
                    <thead>
                      <tr style={{ background: '#F0F2F5' }}>
                        <th style={{ padding: '0.5rem 0.75rem', textAlign: 'left', color: '#717680', fontWeight: 600, fontSize: '0.75rem' }}>Description</th>
                        <th style={{ padding: '0.5rem 0.75rem', textAlign: 'right', color: '#717680', fontWeight: 600, fontSize: '0.75rem' }}>Qty</th>
                        <th style={{ padding: '0.5rem 0.75rem', textAlign: 'left', color: '#717680', fontWeight: 600, fontSize: '0.75rem' }}>Unit</th>
                        <th style={{ padding: '0.5rem 0.75rem', textAlign: 'right', color: '#717680', fontWeight: 600, fontSize: '0.75rem' }}>Est. Cost</th>
                      </tr>
                    </thead>
                    <tbody>
                      {viewing.lineItems.map((item, i) => (
                        <tr key={i} style={{ borderTop: '1px solid #F0F2F5' }}>
                          ...
                          <td style={{ padding: '0.625rem 0.75rem', color: '#717680' }}>{item.unit}</td>
                          <td style={{ padding: '0.625rem 0.75rem', color: '#717680', textAlign: 'right' }}>{item.estimatedCost || '—'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {viewing.adminNote && (
                <div style={{ padding: '0.875rem 1rem', borderRadius: '0.625rem', background: '#F2F3F5', border: '1px solid #D4D7DC' }}>
                  <p style={{ fontSize: '0.75rem', fontWeight: 600, color: '#717680', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.375rem' }}>Admin Note</p>
                  <p style={{ fontSize: '0.875rem', color: '#1B3A5C', lineHeight: 1.6 }}>{viewing.adminNote}</p>
                </div>
              )}
              <p style={{ fontSize: '0.75rem', color: '#B0B4B9' }}>Submitted by {viewing.user.name} on {new Date(viewing.createdAt).toLocaleDateString('en-AU', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
            </div>
      ...
      {showForm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', padding: '0', zIndex: 50 }}>
          <div style={{ background: '#fff', borderRadius: '1rem', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', width: '100%', maxWidth: '44rem', maxHeight: '92vh', display: 'flex', flexDirection: 'column', border: '1px solid #D4D7DC' }}>
            ...
            <div style={{ padding: '1.5rem', overflowY: 'auto', overscrollBehavior: 'contain', flex: 1, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                  ...
                  <button onClick={addLine} style={{ fontSize: '0.8125rem', color: '#4E7FB5', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}
                    onMouseEnter={e => (e.currentTarget.style.color = '#1B3A5C')} onMouseLeave={e => (e.currentTarget.style.color = '#4E7FB5')}>
                    + Add Item
                  </button>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                  {lineItems.map((line, i) => (
                    <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 80px 80px 100px 32px', gap: '0.5rem', alignItems: 'center' }}>
                      ...
                      <button onClick={() => removeLine(i)} disabled={lineItems.length === 1} style={{ background: 'none', border: 'none', cursor: lineItems.length === 1 ? 'not-allowed' : 'pointer', color: '#B0B4B9', padding: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                        onMouseEnter={e => { if (lineItems.length > 1) (e.currentTarget as HTMLButtonElement).style.color = '#DC2626' }} onMouseLeave={e => (e.currentTarget.style.color = '#B0B4B9')}>
