...
import NCRAdmin from './NCRAdmin'

export default async function AdminNCRsPage() {
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#EEF3F9' }}>
      <header style={{ background: '#1B3A5C', padding: '0 1.5rem', height: '3.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        ...
        <span style={{ color: '#A8C4E0', fontSize: '0.875rem' }}>Manage NCRs</span>
      </header>
      <div style={{ maxWidth: '56rem', margin: '0 auto', padding: '2rem 1.5rem' }}>
        <NCRAdmin initialNCRs={ncrs.map((n: typeof ncrs[number]) => ({ ...n, createdAt: n.createdAt.toISOString(), updatedAt: n.updatedAt.toISOString() }))} />
      </div>
    </main>
  )
}
