    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '4rem 1rem', background: '#fff', borderRadius: '1rem', border: '1px solid #D4D7DC' }}>
          <p style={{ color: '#717680', fontSize: '0.9375rem' }}>No change requests match the current filters.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
          {filtered.map(req => {
            ...
            return (
              <div
                key={req.id}
                style={{ background: '#fff', borderRadius: '0.875rem', padding: '1rem 1.25rem', border: '1px solid #D4D7DC', display: 'flex', alignItems: 'flex-start', gap: '1rem' }}
              >
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.375rem', flexWrap: 'wrap' }}>
                    ...
                    <span style={{ fontSize: '0.75rem', color: '#8A8F96' }}>{req.user.name}</span>
                    <span style={{ fontSize: '0.75rem', color: '#B0B4B9', marginLeft: 'auto' }}>
                  ...
                  {req.adminNote && (
                    <p style={{ fontSize: '0.75rem', color: '#717680', marginTop: '0.25rem' }}>Note: {req.adminNote}</p>
                  )}
                </div>
                <div style={{ display: 'flex', gap: '0.625rem', flexShrink: 0, alignItems: 'center' }}>
                  <button
                    onClick={() => openReview(req)}
                    style={{ fontSize: '0.8125rem', fontWeight: 500, color: '#4E7FB5', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
      ...
      {viewing && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', zIndex: 50 }}>
          <div style={{ background: '#fff', borderRadius: '1rem', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', width: '100%', maxWidth: '42rem', maxHeight: '90vh', display: 'flex', flexDirection: 'column', border: '1px solid #D4D7DC' }}>
            <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #F0F2F5', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
              <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
                ...
                <span style={{ fontSize: '0.8125rem', color: '#717680' }}>by {viewing.user.name}</span>
              </div>
            ...
            <div style={{ padding: '1.5rem', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              {/* Admin controls */}
              <div style={{ borderTop: '1px solid #F0F2F5', paddingTop: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                <div>
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 600, color: '#4E7FB5', marginBottom: '0.375rem' }}>Admin Note <span style={{ fontWeight: 400, color: '#8A8F96' }}>(optional)</span></label>
                  <textarea
                    value={editNote}
            ...
            <div style={{ padding: '1rem 1.5rem', borderTop: '1px solid #F0F2F5', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
              <button
