...
const TDS_CATEGORIES = [
  'Resins', 'Adhesives', 'Coatings', 'Composites',
  'Reinforcements', 'Core Materials', 'Solvents', 'General',
]

const CATEGORY_COLORS: Record<string, { bg: string; icon: string; border: string }> = {
  'Resins':            { bg: '#DCEAF7', icon: '#3D6B9B', border: '#BFDBFE' },
  ...
  'Reinforcements':    { bg: '#DCEAF7', icon: '#1D4ED8', border: '#BFDBFE' },
      ...
      <main style={{ maxWidth: '72rem', margin: '0 auto', padding: '2rem 1rem' }}>
        ...
        {showUpload && (
          <div style={{ background: '#fff', borderRadius: '1rem', border: '1px solid #D0DCE8', padding: '1.5rem', marginBottom: '1.75rem', boxShadow: '0 2px 8px rgba(27,58,92,0.06)' }}>
            ...
            {/* Drop zone */}
            <div style={{ border: '2px dashed #C5D8EF', borderRadius: '0.875rem', padding: '2rem 1rem', textAlign: 'center', marginBottom: '1rem', background: uploadFiles && uploadFiles.length > 0 ? '#EEF7FF' : '#F8FAFC' }}>
              <svg style={{ width: '2rem', height: '2rem', color: '#3D6B9B', margin: '0 auto 0.75rem' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
              </svg>

              {uploadFiles && uploadFiles.length > 0 ? (
                <p style={{ fontWeight: 600, color: '#3D6B9B', fontSize: '0.9375rem', marginBottom: '1rem' }}>
              ...
              <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'center', flexWrap: 'wrap' }}>
                <button
                  ...
                  style={{ padding: '0.5rem 1.125rem', fontSize: '0.875rem', fontWeight: 600, background: '#fff', color: '#3D6B9B', border: '1.5px solid #3D6B9B', borderRadius: '0.5rem', cursor: 'pointer' }}
            ...
            {uploadResult && (
              <div style={{ padding: '0.75rem 1rem', borderRadius: '0.625rem', marginBottom: '1rem', background: uploadResult.failed === 0 ? '#EEF7FF' : '#FFF7ED', border: `1px solid ${uploadResult.failed === 0 ? '#BFDBFE' : '#FED7AA'}` }}>
                <p style={{ fontWeight: 600, fontSize: '0.875rem', color: uploadResult.failed === 0 ? '#3D6B9B' : '#D97706', marginBottom: uploadResult.failed > 0 ? '0.5rem' : 0 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button
                ...
                style={{ padding: '0.5625rem 1.5rem', background: '#3D6B9B', color: '#fff', border: 'none', borderRadius: '0.5rem', fontWeight: 600, fontSize: '0.875rem', cursor: uploading ? 'not-allowed' : 'pointer', opacity: (uploading || !uploadFiles || uploadFiles.length === 0) ? 0.6 : 1 }}
        ...
        {docs.length > 0 && (
          <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
            <div style={{ position: 'relative', flex: '1 1 200px', minWidth: '160px' }}>
              ...
              <input type="text" placeholder="Search…" value={search} onChange={e => setSearch(e.target.value)} style={{ width: '100%', paddingLeft: '2.25rem', paddingRight: '0.75rem', paddingTop: '0.4375rem', paddingBottom: '0.4375rem', borderRadius: '0.5rem', border: '1px solid #C5D8EF', color: '#1B3A5C', fontSize: '0.875rem', background: '#fff', boxSizing: 'border-box' }} />
            </div>
            <select value={filterCategory} onChange={e => setFilterCategory(e.target.value)} style={{ padding: '0.4375rem 0.75rem', borderRadius: '0.5rem', border: '1px solid #C5D8EF', color: '#1B3A5C', fontSize: '0.875rem', background: '#fff' }}>
        ...
        {docs.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '4rem 1rem', background: '#fff', borderRadius: '1rem', border: '1px solid #D0DCE8' }}>
            <p style={{ color: '#6B7A8D', fontSize: '0.9375rem' }}>No TDS documents uploaded yet. Use the upload panel above.</p>
          </div>
        ) : filtered.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            {filtered.map(doc => {
              ...
              return (
                <div key={doc.id} style={{ background: '#fff', borderRadius: '0.875rem', padding: '0.875rem 1.125rem', border: '1px solid #D0DCE8', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: '0.9375rem', fontWeight: 600, color: '#1B3A5C', marginBottom: '1px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{doc.title}</p>
                    <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
                      <span style={{ fontSize: '0.6875rem', fontWeight: 600, padding: '1px 6px', borderRadius: '9999px', background: cs.bg, color: cs.icon, border: `1px solid ${cs.border}` }}>{doc.category}</span>
                      {doc.manufacturer && <span style={{ fontSize: '0.75rem', color: '#6B7A8D' }}>{doc.manufacturer}</span>}
                      <span style={{ fontSize: '0.75rem', color: '#B0BAC5' }}>{formatFileSize(doc.fileSize)}</span>
                    </div>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexShrink: 0 }}>
                    <a href={doc.filePath} target="_blank" rel="noopener noreferrer" style={{ fontSize: '0.8125rem', fontWeight: 500, color: '#3D6B9B', textDecoration: 'none' }}
                      onMouseEnter={e => (e.currentTarget.style.color = '#1B3A5C')}
                      onMouseLeave={e => (e.currentTarget.style.color = '#3D6B9B')}
                    >View</a>
                    <button onClick={() => openEdit(doc)} style={{ fontSize: '0.8125rem', fontWeight: 500, color: '#6B7A8D', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
                      onMouseEnter={e => (e.currentTarget.style.color = '#1B3A5C')}
                      onMouseLeave={e => (e.currentTarget.style.color = '#6B7A8D')}
                    >Edit</button>
                    <button onClick={() => handleDelete(doc.id)} disabled={deletingId === doc.id} style={{ fontSize: '0.8125rem', fontWeight: 500, color: '#DC2626', background: 'none', border: 'none', cursor: 'pointer', padding: 0, opacity: deletingId === doc.id ? 0.5 : 1 }}
          ...
          <div style={{ background: '#fff', borderRadius: '1rem', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', width: '100%', maxWidth: '32rem', border: '1px solid #D0DCE8' }}>
            ...
            <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                ...
                <input type="text" value={editTitle} onChange={e => setEditTitle(e.target.value)} style={{ width: '100%', padding: '0.5rem 0.75rem', borderRadius: '0.5rem', border: '1px solid #C5D8EF', color: '#1B3A5C', fontSize: '0.875rem', boxSizing: 'border-box' }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 600, color: '#3D6B9B', marginBottom: '0.375rem' }}>Manufacturer <span style={{ fontWeight: 400, color: '#8B939E' }}>(optional)</span></label>
                <input type="text" value={editManufacturer} onChange={e => setEditManufacturer(e.target.value)} style={{ width: '100%', padding: '0.5rem 0.75rem', borderRadius: '0.5rem', border: '1px solid #C5D8EF', color: '#1B3A5C', fontSize: '0.875rem', boxSizing: 'border-box' }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.875rem', fontWeight: 600, color: '#3D6B9B', marginBottom: '0.375rem' }}>Category</label>
                <select value={editCategory} onChange={e => setEditCategory(e.target.value)} style={{ width: '100%', padding: '0.5rem 0.75rem', borderRadius: '0.5rem', border: '1px solid #C5D8EF', color: '#1B3A5C', fontSize: '0.875rem', background: '#fff' }}>
                  {TDS_CATEGORIES.map(c => <option key={c}>{c}</option>)}
                </select>
              </div>
            </div>
            <div style={{ padding: '1rem 1.5rem', borderTop: '1px solid #EEF3F9', display: 'flex', justifyContent: 'flex-end', gap: '0.625rem' }}>
              <button onClick={() => setEditing(null)} style={{ padding: '0.5rem 1rem', fontSize: '0.875rem', fontWeight: 500, color: '#6B7A8D', background: 'none', border: 'none', cursor: 'pointer' }}>Cancel</button>
              <button onClick={handleSaveEdit} disabled={saving} style={{ padding: '0.5rem 1.25rem', fontSize: '0.875rem', fontWeight: 600, background: '#3D6B9B', color: '#fff', border: 'none', borderRadius: '0.5rem', cursor: 'pointer', opacity: saving ? 0.6 : 1 }}>
