    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '4rem 1rem', background: '#fff', borderRadius: '1rem', border: '1px solid #D4D7DC' }}>
          <p style={{ color: '#717680' }}>No checklists match this filter.</p>
        </div>
      ) : (
          ...
          {filtered.map(cl => {
            ...
            return (
              <div key={cl.id} style={{ background: '#fff', borderRadius: '0.875rem', padding: '1rem 1.25rem', border: `1px solid ${isPass ? '#BBF7D0' : '#FECACA'}` }}>
                ...
                <p style={{ fontSize: '0.8125rem', color: '#717680', marginBottom: '0.75rem' }}>{cl.customerName} · {done}/{parsed.length} checks · by {cl.user.name}</p>
            ...
            <div style={{ background: '#fff', borderRadius: '1rem', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', width: '100%', maxWidth: '38rem', maxHeight: '85vh', display: 'flex', flexDirection: 'column', border: '1px solid #D4D7DC' }}>
              <div style={{ padding: '1.25rem 1.5rem', borderBottom: '1px solid #F0F2F5', display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem', flexShrink: 0 }}>
                <div>
                  ...
                  <p style={{ fontSize: '0.8125rem', color: '#717680' }}>Submitted by {viewing.user.name}</p>
                </div>
      ...
      {deleting && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', zIndex: 50 }}>
          <div style={{ background: '#fff', borderRadius: '1rem', padding: '1.5rem', maxWidth: '24rem', width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            ...
            <p style={{ fontSize: '0.875rem', color: '#717680', marginBottom: '1.25rem' }}>Checklist for SO <strong>{deleting.bcSoNumber}</strong> will be permanently deleted.</p>
