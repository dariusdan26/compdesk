...
const STATUS_STYLES: Record<string, { bg: string; color: string }> = {
  ...
  ordered:  { bg: '#E0E4E9', color: '#4E7FB5' },
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.75rem', gap: '1rem', flexWrap: 'wrap' }}>
        <div>
          ...
          <p style={{ color: '#717680', fontSize: '0.875rem' }}>{openCount} open · {requisitions.length} total</p>
        </div>
      ...
      {filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '4rem 1rem', background: '#fff', borderRadius: '1rem', border: '1px solid #D4D7DC' }}>
          <p style={{ color: '#717680' }}>No requisitions match this filter.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          {filtered.map(req => {
            ...
            return (
              <div key={req.id} style={{ background: '#fff', borderRadius: '0.875rem', padding: '1rem 1.25rem', border: '1px solid #D4D7DC' }}>
                ...
                <p style={{ fontSize: '0.8125rem', color: '#717680', marginBottom: '0.75rem' }}>
            ...
            <div style={{ padding: '1.5rem', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '1.125rem' }}>
              ...
              <div>
                ...
                <textarea value={reviewForm.adminNote} onChange={e => setReviewForm(f => ({ ...f, adminNote: e.target.value }))} placeholder="Add BC PO number, approval notes, or rejection reason..." rows={3}
                  style={{ width: '100%', padding: '0.5rem 0.75rem', borderRadius: '0.5rem', border: '1px solid #C8CDD3', color: '#1B3A5C', fontSize: '0.875rem', background: '#fff', resize: 'vertical', fontFamily: 'inherit', boxSizing: 'border-box' }} />
              </div>
            </div>
      ...
      {deleting && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', zIndex: 50 }}>
          <div style={{ background: '#fff', borderRadius: '1rem', padding: '1.5rem', maxWidth: '24rem', width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            ...
            <p style={{ fontSize: '0.875rem', color: '#717680', marginBottom: '1.25rem' }}><strong>{deleting.title}</strong> and all its line items will be permanently deleted.</p>
